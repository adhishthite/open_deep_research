...
async def load_mcp_tools(
    ...
    try:
        ...
        warnings.warn(f"Error loading MCP tools: {e}")
        return []
    for mcp_tool in mcp_tools:
        ...
        if mcp_tool.name not in set(configurable.mcp_config.tools):
            continue
        tools.append(wrap_mcp_authenticate_tool(mcp_tool))
    return tools
...
async def get_search_tool(search_api: SearchAPI):
    """Get search tool configuration based on the specified search API."""
...
async def get_all_tools(config: RunnableConfig):
    """Get all available tools including search tools based on configuration."""
...
def get_notes_from_tool_calls(messages: list[MessageLikeRepresentation]):
    """Extract tool call content from message list."""
    return [
...
def is_token_limit_exceeded(exception: Exception, model_name: str = None) -> bool:
    """Check if the exception indicates a token limit was exceeded."""
...
def remove_up_to_last_ai_message(
    ...
    """Remove messages up to the last AI message in the list."""
...
def get_config_value(value):
    """Get configuration value, handling None and string conversion."""
    if value is None:
        return None
...
def get_api_key_for_model(model_name: str, config: RunnableConfig):
    """Get API key for a specific model from configuration."""
